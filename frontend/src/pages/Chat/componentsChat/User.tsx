      ...
      <div className='users'>
        {
          findUser.map((user) => {
            return (
              <AddFriend username={user?.username} status={user?.status} avatar={user?.avatar} userid={user?.id} login={props?.login} Onchange={props.Onchange} setClickUser={props.setClickUser} clickUser={props.clickUser} key={user?.id}></AddFriend> 
            )
          })
        }
      </div>
